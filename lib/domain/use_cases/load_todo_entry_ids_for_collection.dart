...
class LoadToDoEntryIdsForCollection
    implements UseCase<List<EntryId>, LoadToDoEntryIdsParams> {
  ...
  Future<Either<Failure, List<EntryId>>> call(
      LoadToDoEntryIdsParams params) async {
    try {
      final loadedEntryIds =
          await toDoRepository.readToDoEntryIds(params.collectionId);
      return loadedEntryIds.fold((left) => Left(left), (right) => Right(right));
